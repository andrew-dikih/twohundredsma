...
- `workflow_dispatch` in `ci.yml` and `deploy.yml` is unaffected by
  `paths-ignore` — a manual dispatch always runs regardless of the last
  diff's contents. This is the intended manual recovery control for the
  >=3,000-file limitation below.
...
diff](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#git-diff-comparisons).
...
- **Mitigation:** split oversized changes into smaller PRs/pushes. If an
  ...
  releases and manually run `workflow_dispatch` on the affected workflows
  (`ci.yml` and, if a deploy may be needed, `deploy.yml`) to force a real
  validation/deploy run rather than trusting the skip. Both workflows'
  `workflow_dispatch` triggers are unaffected by `paths-ignore` and run
  regardless of the last diff's contents.
